...
test('incomplete consumer fields and impossible measurements never publish or enter Finder',()=>{
  const complete={...fish,aliases:[],similar:[]};
  assert.equal(isPublishable(complete),true);
  for(const change of [{colors:undefined},{colors:['purple']},{aliases:null},{similar:[42]},{community:undefined},{breedingDifficulty:'unknown'},{size:Infinity},{tankLitres:Infinity},{ph:[-3,100]},{gh:[-1,10]},{temperature:[-20,80]}]) {
    const malformed={...complete,...change};
    assert.equal(isPublishable(malformed),false,JSON.stringify(change));
    assert.doesNotThrow(()=>findMatches(input,[malformed]));
    assert.equal(findMatches(input,[malformed]).matches.length,0);
  }
});
